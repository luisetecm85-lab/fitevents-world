...
useEffect(()=>{const handler=e=>{e.preventDefault();setInstallPrompt(e);};window.addEventListener('beforeinstallprompt',handler);return()=>window.removeEventListener('beforeinstallprompt',handler);},[]);
  ...
  return<div style={{minHeight:"100vh",background:"#0a0a0a",display:"flex",flexDirection:"column"}}>
    ...
    <header style={{background:"#0d0d0d",borderBottom:"1px solid #1e1e1e",padding:isMobile?"0 8px":"0 20px",height:isMobile?56:170,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100,flexShrink:0}}>
      <div style={{position:isMobile?"static":"absolute",left:isMobile?"auto":"50%",transform:isMobile?"none":"translateX(-50%)",flex:isMobile?1:0,display:"flex",justifyContent:"center",cursor:"pointer"}} onClick={()=>setView("home")}><img src="/logo-horizontal.png" alt="FitEvents World" style={{height:isMobile?38:160,width:"auto",maxWidth:isMobile?"180px":"500px"}}/></div>
