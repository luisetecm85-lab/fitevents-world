...
const EVENTS=[
  ...
  {id:129,name:"ELITE FUNCTIONAL CHALLENGER TORROX 2026",disc:"Functional",city:"Torrox",prov:"Málaga",country:"España",date:"2026-04-30",price:200,fmts:["Individual"],desc:"Elite Functional Challenger en Torrox, Málaga.",feat:true,verified:false,ratings:[]},
    ...
    <div style={{display:"flex",flex:1}}>
      ...
      {view==="prof"&&me&&<div style={{maxWidth:700,margin:"0 auto"}}>
        ...
        {evs.filter(e=>e.ratings.find(r=>r.user===me.u)).length>0?<div style={{marginBottom:16}}>
          ...
          {auth.currentUser&&!auth.currentUser.emailVerified&&<div style={{background:"rgba(255,193,7,0.08)",border:"1px solid rgba(255,193,7,0.25)",borderRadius:8,padding:"10px 14px",marginBottom:12,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}><span style={{fontSize:16}}>⚠️</span><div style={{flex:1}}><div style={{fontSize:13,fontWeight:700,color:"#FFB300",marginBottom:2}}>Email no verificado</div><div style={{fontSize:11,color:"#888"}}>Revisa tu bandeja de entrada y verifica tu email para poder valorar eventos.</div></div><button onClick={()=>auth.currentUser.sendEmailVerification().then(()=>alert("Email reenviado"))} style={{background:"rgba(255,193,7,0.12)",color:"#FFB300",border:"1px solid rgba(255,193,7,0.25)",padding:"5px 12px",borderRadius:6,fontSize:11,fontWeight:700,cursor:"pointer",flexShrink:0}}>Reenviar</button></div>}<div style={{fontSize:13,color:"#555",marginBottom:8}}>Todavía no has valorado ningún evento</div>
