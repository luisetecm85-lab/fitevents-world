    ...
    <div style={{display:"flex",flex:1}}>
      ...
      {view==="list"&&<div>
        ...
        <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap",alignItems:"flex-end"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:3}}><span style={{fontSize:10,fontWeight:700,letterSpacing:1.5,color:"#888",textTransform:"uppercase"}}>Disciplina</span><select value={fDisc} onChange={e=>setFDisc(e.target.value)} style={{padding:isMobile?"6px 4px":"8px 10px",minWidth:isMobile?80:120,fontSize:isMobile?11:13}}>{DISCIPLINES.map(d=><option key={d}>{d}</option>)}</select></div>
          <div style={{display:"flex",flexDirection:"column",gap:3}}><span style={{fontSize:10,fontWeight:700,letterSpacing:1.5,color:"#888",textTransform:"uppercase"}}>País</span><select value={fCountry} onChange={e=>{setFCountry(e.target.value);setFCities([]);}} style={{padding:isMobile?"6px 4px":"8px 10px",minWidth:isMobile?80:100,fontSize:isMobile?11:13}}>{countries.map(c=><option key={c}>{c}</option>)}</select></div>
