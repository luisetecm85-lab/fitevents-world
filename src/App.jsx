...
function CalendarView({evs,setSel,setView}){
  ...
  return<div>
    <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:28,fontWeight:900,marginBottom:16,letterSpacing:1,textTransform:"uppercase",textAlign:"center"}}>Calendario</div>
...
useEffect(()=>{const handler=e=>{e.preventDefault();setInstallPrompt(e);};window.addEventListener('beforeinstallprompt',handler);return()=>window.removeEventListener('beforeinstallprompt',handler);},[]);
  ...
  return<div style={{minHeight:"100vh",background:"#0a0a0a",display:"flex",flexDirection:"column"}}>
    ...
    <div style={{display:"flex",flex:1}}>
      ...
      {view==="cal"&&<div style={{display:"flex",gap:8}}>{!isMobile&&<div style={{width:140,flexShrink:0}}><div style={{background:"#161616",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:"14px 16px",position:"sticky",top:80}}><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:12,fontWeight:800,color:"#FF6500",letterSpacing:1,textTransform:"uppercase",marginBottom:12}}>Este mes</div>{[{label:"Eventos",value:evs.filter(e=>{const d=new Date();return e.date&&e.date.startsWith(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`);}).length,color:"#FF6500"},{label:"CrossFit",value:evs.filter(e=>{const d=new Date();return e.disc==="CrossFit"&&e.date&&e.date.startsWith(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`);}).length,color:"#FF6500"},{label:"Hyrox",value:evs.filter(e=>{const d=new Date();return e.disc==="Hyrox"&&e.date&&e.date.startsWith(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`);}).length,color:"#4DA6FF"},{label:"OCR",value:evs.filter(e=>{const d=new Date();return e.disc==="OCR"&&e.date&&e.date.startsWith(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`);}).length,color:"#4CAF50"},{label:"Fuerza",value:evs.filter(e=>{const d=new Date();return e.disc==="Fuerza"&&e.date&&e.date.startsWith(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`);}).length,color:"#B56AFF"},{label:"F. Funcional",value:evs.filter(e=>{const d=new Date();return e.disc==="Fitness Funcional"&&e.date&&e.date.startsWith(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`);}).length,color:"#FFB300"}].map((s,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><span style={{fontSize:11,color:"#888"}}>{s.label}</span><span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:16,fontWeight:800,color:s.color}}>{s.value}</span></div>)}</div></div>}<div style={{flex:1,minWidth:0}}><CalendarView evs={evs} setSel={setSel} setView={setView}/></div>{!isMobile&&<div style={{width:140,flexShrink:0}}><div style={{background:"#161616",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:"14px 16px",position:"sticky",top:80}}><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:12,fontWeight:800,color:"#FF6500",letterSpacing:1,textTransform:"uppercase",marginBottom:12}}>Próximos</div>{evs.filter(e=>e.date>=new Date().toISOString().split("T")[0]).sort((a,b)=>a.date.localeCompare(b.date)).slice(0,5).map((ev,i)=><div key={i} onClick={()=>{setSel(ev);setView("det");}} style={{marginBottom:8,cursor:"pointer"}}><div style={{fontSize:10,fontWeight:700,color:DISC_COLORS[ev.disc]||"#FF6500",marginBottom:1}}>{ev.disc}</div><div style={{fontSize:11,color:"#fff",fontWeight:600,lineHeight:1.2,marginBottom:1}}>{ev.name}</div><div style={{fontSize:10,color:"#555"}}>{ev.city} · {fd(ev.date)}</div></div>)}</div></div>}</div>}{view==="rnk"&&<div>
