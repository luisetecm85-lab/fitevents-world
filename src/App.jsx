...
useEffect(()=>{const handler=e=>{e.preventDefault();setInstallPrompt(e);};window.addEventListener('beforeinstallprompt',handler);return()=>window.removeEventListener('beforeinstallprompt',handler);},[]);
  ...
  return<div style={{minHeight:"100vh",background:"#0a0a0a",display:"flex",flexDirection:"column"}}>
    ...
    <div style={{display:"flex",flex:1}}>
      ...
      {view==="sponsors"&&<div style={{maxWidth:700,margin:"0 auto"}}><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:28,fontWeight:900,marginBottom:6,letterSpacing:1,textTransform:"uppercase"}}>Descuentos exclusivos</div><div style={{fontSize:13,color:"#888",marginBottom:16}}>Códigos exclusivos para usuarios registrados de FitEvents World</div><div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:12}}>{[{brand:"PICSIL",desc:"Equipamiento técnico para atletas de élite",code:"FITEVENTS10",discount:"10% de descuento",color:"#FF6500",logo:"💪",url:"https://picsil.com"},{brand:"ON RUNNING",desc:"Zapatillas diseñadas para rendimiento híbrido",code:"FITON15",discount:"15% de descuento",color:"#4DA6FF",logo:"👟",url:"https://on-running.com"},{brand:"BAREBELLS",desc:"Proteína sin azúcar. Sabor sin compromiso.",code:"FITBAR20",discount:"20% de descuento",color:"#B56AFF",logo:"🍫",url:"https://barebells.com"}].map((s,i)=><div key={i} style={{background:`linear-gradient(135deg,${s.color}10,#1a1a1a)`,border:`1px solid ${s.color}28`,borderRadius:12,padding:"16px 18px"}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}><span style={{fontSize:28}}>{s.logo}</span><div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:18,fontWeight:900,color:s.color}}>{s.brand}</div><div style={{fontSize:11,color:"#888"}}>{s.desc}</div></div></div><div style={{background:"rgba(0,0,0,0.3)",borderRadius:8,padding:"8px 12px",marginBottom:10,display:"flex",alignItems:"center",justifyContent:"space-between"}}><div><div style={{fontSize:10,color:"#555",fontWeight:700,letterSpacing:1,textTransform:"uppercase",marginBottom:2}}>Tu código</div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:20,fontWeight:900,color:"#fff",letterSpacing:2}}>{s.code}</div></div><button onClick={()=>{navigator.clipboard.writeText(s.code);alert(`Código copiado: ${s.code}`);}} style={{background:s.color,color:"#fff",border:"none",padding:"6px 12px",borderRadius:6,fontSize:11,fontWeight:700,cursor:"pointer"}}>Copiar</button></div><div style={{fontSize:12,color:"#4CAF50",fontWeight:700,marginBottom:8}}>✓ {s.discount}</div><a href={s.url} target="_blank" rel="noopener noreferrer" style={{color:s.color,fontSize:11,fontWeight:700,textDecoration:"none"}}>Ir a la tienda →</a></div>)}</div></div>}{(view==="legal-privacy"||view==="legal-terms"||view==="legal-cookies"||view==="legal-aviso")&&<div style={{maxWidth:720,margin:"0 auto"}}>
